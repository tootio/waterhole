...
module Authentication
  ...
  included do
    ...
    # Runs for signed-out visitors too, as a no-op, so public pages cannot
    # become a way around it for a signed-in moderator.
  ...
  class_methods do
    ...
    # For the few pages a moderator must reach before consenting: the consent
    # page itself, the documents it asks them to read, and signing out.
    def allow_without_consent(**options)
      skip_before_action :require_consent, **options
    end
  end
  ...
  # Until a signed-in moderator agrees to the processing of their data, every
  # page but the legal documents and the consent page leads to the consent page.
  def require_consent
    return unless signed_in?
    return if current_moderator.consent_current?
  ...
  # remember: whether this browser should offer the instance on the sign-in
  # page next time. Opt-in, chosen when signing in; not choosing it forgets the
  # instance.
  def start_new_session_for(moderator, remember: false)
    ...
    moderator.sessions.create!(
      ...
      if remember
        ...
        forget_recent_instance(moderator.instance.domain)
      end
    end
  end
