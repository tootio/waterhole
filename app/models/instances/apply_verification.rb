  ...
  class ApplyVerification
    ...
    def mark_terms_outdated
      ...
      if !@instance.ever_accepted_terms?
        ...
        # fortnight by publishing a record with no accepted_tos at all.
    ...
    def count_failure
      ...
      @instance.assign_attributes(
        consecutive_verification_failures: failures,
        # Access survives a few strikes in case this is mid-edit, but consent to
        # signals does not: no record read means no consent read.
        signals_opted_in: false,
        verification_detail: @result.detail
      )
      ...
      @instance.revoke!(reason: "DNS record for #{DnsAllowlist::PREFIX}.#{@instance.domain} is no longer published.")
    end
