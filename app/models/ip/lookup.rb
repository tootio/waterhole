  ...
  module Lookup
    ...
    class << self
      def call(address)
        ...
        # Bad input from the instance, not a bug here -- and its message would
        # carry the address to the error tracker.
        nil
      ...
      def reload(name)
        ...
        Rails.error.report(e, handled: true, context: { database: name })
        entries.delete(name)
        nil
      end
