...
class TermsTest < ActionDispatch::IntegrationTest
  ...
  test "the banner links only the documents that changed" do
    accepted = nil
    with_legal_documents { accepted = LegalDocuments.digest }

    with_legal_documents(terms_of_service: "# Terms\n\nBe even more excellent.\n") do
      moderators(:avery).instance.update!(status: "terms_outdated", verified_at: 1.day.ago,
        accepted_terms_digest: accepted, terms_grace_until: 3.days.from_now)
      # Consent is per privacy-policy version; this moderator agreed to this one.
      moderators(:avery).update!(consented_privacy_digest: LegalDocuments.privacy_digest)
      sign_in_as moderators(:avery)
      get root_path

      assert_select "div.bg-amber-100 a[href=?]", terms_path, text: "terms of service"
      assert_select "div.bg-amber-100 a[href=?]", privacy_path, count: 0
    end
  end
