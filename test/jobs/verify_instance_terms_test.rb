...
class VerifyInstanceTermsTest < ActiveSupport::TestCase
  ...
  test "a record accepting the current documents verifies and records the digest" do
    with_legal_documents do
      verify(dns_record(accepted_tos: LegalDocuments.digest))
  ...
  test "a record with no accepted_tos leaves an instance that had accepted in grace" do
    with_legal_documents do
  ...
  test "stale terms never touch the revocation strike counter" do
    with_legal_documents do
      ...
      10.times { verify(dns_record(accepted_tos: "stale" * 12)) }
  ...
  # publishing a record with no accepted_tos.
